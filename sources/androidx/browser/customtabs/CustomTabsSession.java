...
public final class CustomTabsSession {
    ...
    public int postMessage(String message, Bundle extras) {
        int postMessage;
        Bundle extras2 = createBundleWithId(extras);
        synchronized (this.mLock) {
            try {
                try {
                    postMessage = this.mService.postMessage(this.mCallback, message, extras2);
                } catch (RemoteException e) {
                    return -2;
                }
            } catch (Throwable th) {
    ...
    private void addIdToBundle(Bundle bundle) {
        if (this.mId != null) {
            bundle.putParcelable(CustomTabsIntent.EXTRA_SESSION_ID, this.mId);
        }
    }
