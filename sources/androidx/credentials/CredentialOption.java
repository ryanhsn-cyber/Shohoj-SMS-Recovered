...
public abstract class CredentialOption {
    ...
    public CredentialOption(String type, Bundle requestData, Bundle candidateQueryData, boolean isSystemProviderRequired, boolean isAutoSelectAllowed, Set<ComponentName> allowedProviders) {
        ...
        this.requestData.putBoolean("androidx.credentials.BUNDLE_KEY_IS_AUTO_SELECT_ALLOWED", this.isAutoSelectAllowed);
    ...
    public static final class Companion {
        ...
        public final CredentialOption createFrom(String type, Bundle requestData, Bundle candidateQueryData, boolean requireSystemProvider, Set<ComponentName> allowedProviders) {
            GetPublicKeyCredentialOption createFrom$credentials_release;
            Intrinsics.checkNotNullParameter(type, "type");
            Intrinsics.checkNotNullParameter(requestData, "requestData");
            Intrinsics.checkNotNullParameter(candidateQueryData, "candidateQueryData");
            Intrinsics.checkNotNullParameter(allowedProviders, "allowedProviders");
            try {
                if (Intrinsics.areEqual(type, PasswordCredential.TYPE_PASSWORD_CREDENTIAL)) {
                    createFrom$credentials_release = GetPasswordOption.INSTANCE.createFrom$credentials_release(requestData, allowedProviders, candidateQueryData);
                } else {
                    if (Intrinsics.areEqual(type, PublicKeyCredential.TYPE_PUBLIC_KEY_CREDENTIAL)) {
                        ...
                        if (string != null) {
                            switch (string.hashCode()) {
                                case -613058807:
                                    if (string.equals(GetPublicKeyCredentialOption.BUNDLE_VALUE_SUBTYPE_GET_PUBLIC_KEY_CREDENTIAL_OPTION)) {
                                        createFrom$credentials_release = GetPublicKeyCredentialOption.INSTANCE.createFrom$credentials_release(requestData, allowedProviders, candidateQueryData);
                                        break;
                                    }
                                    break;
                            }
