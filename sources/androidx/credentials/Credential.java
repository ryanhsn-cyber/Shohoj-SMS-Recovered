...
public abstract class Credential {
    ...
    public static final class Companion {
        ...
        public final Credential createFrom(String type, Bundle data) {
            PublicKeyCredential createFrom$credentials_release;
            Intrinsics.checkNotNullParameter(type, "type");
            Intrinsics.checkNotNullParameter(data, "data");
            try {
                if (Intrinsics.areEqual(type, PasswordCredential.TYPE_PASSWORD_CREDENTIAL)) {
                    createFrom$credentials_release = PasswordCredential.INSTANCE.createFrom$credentials_release(data);
                } else {
                    if (!Intrinsics.areEqual(type, PublicKeyCredential.TYPE_PUBLIC_KEY_CREDENTIAL)) {
                        throw new FrameworkClassParsingException();
                    }
                    createFrom$credentials_release = PublicKeyCredential.INSTANCE.createFrom$credentials_release(data);
                }
