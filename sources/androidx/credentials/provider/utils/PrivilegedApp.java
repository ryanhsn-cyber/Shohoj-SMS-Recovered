...
public final /* data */ class PrivilegedApp {
    ...
    public static final class Companion {
        ...
        public final List<PrivilegedApp> extractPrivilegedApps$credentials_release(JSONObject jsonObject) {
            ...
            JSONArray appsJsonArray = jsonObject.getJSONArray(PrivilegedApp.APPS_KEY);
            int length = appsJsonArray.length();
            for (int i = 0; i < length; i++) {
                JSONObject appJsonObject = appsJsonArray.getJSONObject(i);
                if (Intrinsics.areEqual(appJsonObject.getString(PrivilegedApp.TYPE_KEY), PrivilegedApp.ANDROID_TYPE_KEY)) {
                    JSONObject jSONObject = appJsonObject.getJSONObject(PrivilegedApp.APP_INFO_KEY);
                    Intrinsics.checkNotNullExpressionValue(jSONObject, "appJsonObject.getJSONObject(APP_INFO_KEY)");
                    apps.add(createFromJSONObject(jSONObject, true));
                }
            }
            return apps;
        }
        ...
        public final PrivilegedApp createFromJSONObject(JSONObject appInfoJsonObject, boolean filterUserDebug) {
            ...
            JSONArray signaturesJson = appInfoJsonObject.getJSONArray(PrivilegedApp.SIGNATURES_KEY);
            Set fingerprints = new LinkedHashSet();
            int length = signaturesJson.length();
            for (int j = 0; j < length; j++) {
                if (!filterUserDebug || !Intrinsics.areEqual("userdebug", signaturesJson.getJSONObject(j).optString(PrivilegedApp.BUILD_KEY)) || Intrinsics.areEqual("userdebug", Build.TYPE)) {
                    String string = signaturesJson.getJSONObject(j).getString(PrivilegedApp.FINGERPRINT_KEY);
                    Intrinsics.checkNotNullExpressionValue(string, "signaturesJson.getJSONOb…etString(FINGERPRINT_KEY)");
                    fingerprints.add(string);
                }
            }
            ...
            return new PrivilegedApp(string2, fingerprints);
        }
    }
}
