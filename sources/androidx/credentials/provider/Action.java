...
public final class Action {
    ...
    public Action(CharSequence title, PendingIntent pendingIntent, CharSequence subtitle) {
    ...
    public static final class Companion {
        ...
        public final Slice toSlice(Action action) {
            ...
            Slice.Builder sliceBuilder = new Slice.Builder(Uri.EMPTY, new SliceSpec("Action", 0)).addText(title, null, CollectionsKt.listOf(Action.SLICE_HINT_TITLE)).addText(subtitle, null, CollectionsKt.listOf(Action.SLICE_HINT_SUBTITLE));
            sliceBuilder.addAction(pendingIntent, new Slice.Builder(sliceBuilder).addHints(Collections.singletonList(Action.SLICE_HINT_PENDING_INTENT)).build(), null);
        ...
        public final Action fromSlice(Slice slice) {
            ...
            for (Object element$iv : $this$forEach$iv) {
                ...
                if (it.hasHint(Action.SLICE_HINT_TITLE)) {
                    Object text = it.getText();
                    Intrinsics.checkNotNullExpressionValue(text, "it.text");
                    title = text;
                } else if (it.hasHint(Action.SLICE_HINT_SUBTITLE)) {
                    charSequence = it.getText();
                } else if (it.hasHint(Action.SLICE_HINT_PENDING_INTENT)) {
                    pendingIntent = it.getAction();
                }
            }
            try {
                Intrinsics.checkNotNull(pendingIntent);
                return new Action((CharSequence) title, pendingIntent, charSequence);
            } catch (Exception e) {
