...
public final class ShaderBrushSpan extends CharacterStyle implements UpdateAppearance {
    ...
    public ShaderBrushSpan(ShaderBrush shaderBrush, float alpha) {
        ...
        this.alpha = alpha;
        this.size = Size.INSTANCE.m2480getUnspecifiedNHjbRc();
    }
    ...
    public final void m4677setSizeuvyYCjk(long j) {
        this.size = j;
    }
    ...
    public void updateDrawState(TextPaint textPaint) {
        ...
        if ($this$isUnspecified$iv == Size.INSTANCE.m2480getUnspecifiedNHjbRc()) {
            return;
        }
        Pair finalCachedShader = this.cachedShader;
        if (finalCachedShader == null || !Size.m2468equalsimpl0(finalCachedShader.getFirst().getPackedValue(), this.size)) {
            shader = this.shaderBrush.mo2611createShaderuvyYCjk(this.size);
        } else {
            shader = finalCachedShader.getSecond();
        }
        textPaint.setShader(shader);
        this.cachedShader = TuplesKt.to(Size.m2460boximpl(this.size), shader);
    }
}
