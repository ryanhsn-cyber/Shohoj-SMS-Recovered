...
final /* data */ class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public long mo888calculatePositionllwVHH4(IntRect anchorBounds, long windowSize, LayoutDirection layoutDirection, long popupContentSize) {
        ...
        if (IntSize.m5051getWidthimpl(popupContentSize) + x > IntSize.m5051getWidthimpl(windowSize) && (x = anchorBounds.getLeft() - IntSize.m5051getWidthimpl(popupContentSize)) < 0) {
            x = anchorBounds.getLeft() + ((anchorBounds.getWidth() - IntSize.m5051getWidthimpl(popupContentSize)) / 2);
        }
        int y = (anchorBounds.getTop() - IntSize.m5050getHeightimpl(popupContentSize)) - this.tooltipAnchorPadding;
        if (y < 0) {
