...
public final class RippleHostView extends View {
    ...
    public final void m976addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m977updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            ripple.setHotspot(Offset.m2403getXimpl(interaction.getPressPosition()), Offset.m2404getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m977updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple ripple = this.ripple;
        if (ripple == null) {
            return;
        }
        ripple.trySetRadius(radius);
        ripple.m984setColorDxMtmZc(color, alpha);
        Rect newBounds = new Rect(0, 0, MathKt.roundToInt(Size.m2472getWidthimpl(size)), MathKt.roundToInt(Size.m2469getHeightimpl(size)));
