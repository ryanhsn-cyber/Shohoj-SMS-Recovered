    ...
    public static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            ...
            try {
                ...
                if (rootPath2.endsWith(DomExceptionUtils.SEPARATOR)) {
                    path = path2.substring(rootPath2.length());
                } else {
                    path = path2.substring(rootPath2.length() + 1);
                }
                return new Uri.Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode(mostSpecific.getKey()) + '/' + Uri.encode(path, DomExceptionUtils.SEPARATOR)).build();
            } catch (IOException e) {
