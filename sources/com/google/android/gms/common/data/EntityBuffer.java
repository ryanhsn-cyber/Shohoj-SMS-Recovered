    ...
    public final T get(int position) {
        ...
        if (position >= 0 && position != this.zab.size()) {
            if (position == this.zab.size() - 1) {
                intValue = ((DataHolder) Preconditions.checkNotNull(this.mDataHolder)).getCount() - this.zab.get(position).intValue();
            } else {
                intValue = this.zab.get(position + 1).intValue() - this.zab.get(position).intValue();
            }
            if (intValue == 1) {
